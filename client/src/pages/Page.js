...
const Page = () => {
	...
	useEffect(() => {
		...
		return () => {
			if(isotope.current) isotope.curent?.destroy();
		}
	}, [page])
	...
	useEffect(() => {
		if(page === 'portfolio') {
			filterKey === '*'
			  ...
			  : isotope?.current?.arrange({filter: `.filter-${filterKey}`})
		}
	}, [page, filterKey])
				    ...
				    <div className="container">
				        <div className="isotope-layout">
				            ...
				            <ul className="portfolio-filters isotope-filters" data-aos="fade-up" data-aos-delay="100">
		                        <li data-filter="*" onClick={handleFilterKeyChange('*')} className={`${ filterKey === '*' ? 'filter-active' : '*' }`}>All</li>
		                        <li data-filter=".filter-app" onClick={handleFilterKeyChange('App')} className={`${ filterKey === 'App' ? 'filter-active' : '' }`}>App</li>
		                        <li data-filter=".filter-product" onClick={handleFilterKeyChange('Card')} className={`${ filterKey === 'Card' ? 'filter-active' : '' }`}>Card</li>
		                        <li data-filter=".filter-branding" onClick={handleFilterKeyChange('Web')} className={`${ filterKey === 'Web' ? 'filter-active' : '' }`}>Web</li>
				            </ul>

				            <div className="row gy-4 portfolio-container" data-aos="fade-up" data-aos-delay="200" ref={portfolioContainerRef}>
							{
								portfolios.map((portfolio, idx) => {
									return (
										<div key={idx} className={`col-md-4 col-md-6 portfolio-item filter-${ filterKey !== '*' ? portfolio.name : '*' }`}>
										    <img src={`${window.location.origin}${portfolio.image}`} className="img-fluid" alt={portfolio.name} />
										    <div className="portfolio-info">
										        <h4>{portfolio.name}</h4>
										        <p>{portfolio.description}</p>
										        <Link to={`${window.location.origin}${portfolio.image}`} data-title={`${portfolio.name}`} data-description={portfolio.description} className="glightbox preview-link"><i className="bi bi-zoom-in"></i></Link>
